      ...
      <div className="innerContainer">
        ...
        <div className="Links">
          ...
          <div className="dropdown"  onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
            ...
            {/* Dropdown content */}
            <div className={`dropdown-content ${dropdownOpen ? 'show' : ''}`} onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
              <Link className="Header_Links" to="/" onClick={handleLinkClick}>HOME</Link>
              {authenticated && userDetails ? (
                <>
                  {userDetails.role_id.roleName === 'student' && (
                    <Link className="Header_Links" to="/studentInfo" onClick={handleLinkClick}>Student Info</Link>
                  )}
                  {userDetails.role_id.roleName === 'admin' && (
                    <Link className="Header_Links" to="/table" onClick={handleLinkClick}>TABLE</Link>
                  )}
                  {userDetails.role_id.roleName === 'teacher' && (
                    <Link className="Header_Links" to="/table" onClick={handleLinkClick}>TABLE</Link>
                  )}
                  {userDetails.role_id.roleName === 'admin' && (
                    <Link className="Header_Links" to="/users" onClick={handleLinkClick}>Users</Link>
                  )}
                  ...
                  <div className="Header-Userdetails">
                    ...
                    <div className="Header_Links_Button" onClick={() => {handleLogout(); handleLinkClick(); }}>
                      <strong><IoLogOut /></strong>
                    </div>
                  </div>
                </>
              ) : (
                <Link className="Header_Links" to="/login" onClick={handleLinkClick }>Login</Link>
              )}
            </div>
          </div>
